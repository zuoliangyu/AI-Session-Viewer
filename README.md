...
- 消息页新增多会话分屏、线程视图、历史追问分叉与当前会话提问定位悬浮球，长对话浏览和定位更直接。
- 全局搜索补齐 `all / session / content` 范围过滤、会话名/别名命中、标签筛选与会话分组联动。
- Chat/CLI 对话链路修复了新会话与继续会话的状态串扰，并让 Codex 默认模型正确接入自动选型。
- 消息加载、线程挂载、Markdown 与代码高亮都做了新一轮性能优化，大会话打开与重复进入的体感更轻。

...
- 消息页支持多 session 分屏对比，可在左右分屏与上下分屏之间切换；横向布局下补充拖动滚动，长会话并排查看更顺手。
- `AskUserQuestion` 工具结果支持直接在界面内填写并提交，页面失焦时可通过标题与系统通知接收网页/Claude Code 新回复提醒，同时新增全部展开/折叠与 Bash 命令去重，减少流式消息噪音。
- 搜索与消息浏览体验继续打磨：全局搜索支持按会话名/内容范围筛选并联动标签过滤，搜索结果与会话页可直接复制会话名，消息列表补充日期时间切换等显示细节优化。

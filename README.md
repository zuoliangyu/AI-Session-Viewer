...
- 修复桌面端与开发模式下因前端 API 启动链路使用顶层 `await` 导致的白屏问题，应用现在可正常进入主界面。
- `src/services/api.ts` 改为同步导出的懒加载代理，避免 WebView 在模块初始化阶段直接因语法/运行时兼容性中断。
- `2.8.0` 引入的多会话分屏、线程视图、搜索增强与性能优化保持不变，本次为稳定性修复版本。

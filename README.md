...
- **左侧提问目录侧栏**：会话内所有用户提问按顺序列出，点击跳转并高亮对应消息；展开 240px、折叠 40px，状态持久化。
- **Thread 摘要视图**：顶栏按钮切换，把会话展平为"用户提问 → CC 回复摘要"卡片列表，方便快速定位。
- **消息级整体折叠与思考过程默认折叠**：全部折叠按钮现在真的把每条消息缩为一行预览；每个思考过程块独立可展开，默认折叠。
- **划选文本 Reply 浮动按钮**：在消息区划选任意文字即弹出胶囊按钮，点击以 markdown 引用注入续聊输入框。
- **首问粘性横幅**：消息滚动区顶部常驻显示当前已加载消息的首条提问，作为上下文锚点，滚动不消失。
- **修复复制按钮**：统一 `copyTextToClipboard`（await + `execCommand` 回退），告别静默失败。
- **修复续聊完成后需手动刷新**：流式完成后主动 reload 最新消息并清空续聊 pane，历史列表立即接管。
- **修复多处布局与体验问题**：进入会话不再反复自动补页；TOC 点击可达且不再跟外层 ScrollArea 一起卷走（左右各自独立滚动）。
